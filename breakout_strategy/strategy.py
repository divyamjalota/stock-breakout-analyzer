...
def backtest_breakout_strategy_with_capital_constraint(stock_data, volume_multiplier=2, min_daily_return=0.02, holding_period=10, trailing_stop_loss=0, wait_period=0):
    """
    Backtest the breakout strategy with capital constraints (only one active trade at a time).
    ...
    try:
        ...
        max_drawdown = 0.0
        equity_curve = []
        active_trade_end_date = None  # Tracks when the current trade ends

        for i, row in stock_data.iterrows():
            # Skip if a trade is currently active
            current_date = i
            if active_trade_end_date and current_date <= active_trade_end_date:
                continue
...
def backtest_breakout_strategy_without_constraint(stock_data, volume_multiplier=2, min_daily_return=0.02, holding_period=10, trailing_stop_loss=0, wait_period=0):
    """
    Backtest the breakout strategy without capital constraints (multiple trades allowed).
